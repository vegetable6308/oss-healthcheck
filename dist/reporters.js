...
export function renderText(report, colors = true) {
    ...
    for (const result of report.results) {
        ...
        if (!result.passed)
            lines.push(`    ${result.message}`, `    Fix: ${result.remediation}`);
    }
    lines.push("", `${String(report.summary.passed)} passed, ${String(report.summary.failed)} failed, ${String(report.summary.disabled)} disabled`);
    return `${lines.join("\n")}\n`;
}
...
export function renderReport(report, format, colors = true) {
    if (format === "json")
...
export function assertSafeOutputPath(root, outputPath) {
