import type { RepositoryContext, Rule } from "./types.js";

function pass(message: string, evidence: string[]) {
  return { passed: true, message, evidence };
}

function fail(message: string) {
  return { passed: false, message, evidence: [] as string[] };
}

function firstExisting(context: RepositoryContext, candidates: string[]): string | undefined {
  return candidates.find((candidate) => context.has(candidate));
}

function matching(context: RepositoryContext, pattern: RegExp): string[] {
  return context.find(pattern);
}

function packageScripts(context: RepositoryContext): Record<string, string> {
...
export const rules: Rule[] = [
  {
    ...
    async evaluate(context) {
      ...
      if (file === undefined) return fail("No README found.");
      const content = (await context.read(file)) ?? "";
      const hasCoreSections = [/install/i, /usage|quick ?start/i, /contribut/i].filter((r) =>
        r.test(content),
      ).length;
  ...
  },
  {
    ...
    async evaluate(context) {
      ...
      if (files.length === 0) return fail("No license file found.");
  ...
  },
  {
    ...
    async evaluate(context) {
      ...
      if (files.length === 0) return fail("No security policy found.");
  ...
  },
  {
    ...
    weight: 4,
    remediation:
      "Add structured bug report and feature request templates under .github/ISSUE_TEMPLATE.",
    async evaluate(context) {
  ...
  },
  {
    ...
    async evaluate(context) {
      const testFiles = matching(
        context,
        /(^|\/)(test|tests|spec|__tests__)(\/|\.)|\.(test|spec)\.[^/]+$/i,
      );
      const scripts = packageScripts(context);
      const commands = Object.entries(scripts).filter(([name, command]) =>
        /test/i.test(`${name} ${command}`),
      );
  ...
  },
  {
    ...
    async evaluate(context) {
      const files = matching(
        context,
        /(^|\/)(eslint\.config\.|\.eslintrc|pyproject\.toml$|ruff\.toml$|\.golangci|rustfmt\.toml$|\.prettierrc)/i,
      );
      const scripts = packageScripts(context);
  ...
  },
  {
    ...
    async evaluate(context) {
      const files = matching(
        context,
        /(^|\/)(tsconfig.*\.json|pyrightconfig\.json|mypy\.ini|\.golangci.*|clippy\.toml)$/i,
      );
    ...
    async evaluate(context) {
      if (context.packageJson === undefined) {
        const alternatives = matching(
          context,
          /(^|\/)(pyproject\.toml|cargo\.toml|go\.mod|pom\.xml)$/i,
        );
      ...
      return present.length === required.length
        ? pass(
            "Package metadata links users to source, license, and support.",
            present.map((key) => `package.json#${key}`),
          )
        : fail(
            `Package metadata is missing: ${required.filter((key) => !present.includes(key)).join(", ")}.`,
          );
    },
  },
  {
